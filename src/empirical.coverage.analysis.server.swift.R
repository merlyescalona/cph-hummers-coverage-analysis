...
offtargetown=paste0(WD2,"files/offtarget.swift.bed")
targetsBedFile=paste0(WD2,"files/targets.swift.2.bed")
GFF=paste0(WD2,"files/Chaetura_pelagica.CDS.2.gff")
...
# for (i in c(1,48)) {
  ...
  colnames(data)=c("scaffold","start","end","gene","cov1","nmbases","size","breadth", "targetname")
...
files2 <- list.files(
path = paste0(WD,"bedtools2/hist2/"),
pattern="*.gz$")
...
for (i in 1:length(filesWO)) {
  ...
  s=split(data, data$targetname)
  sumCoveragePerTarget=sapply(s,function(x){sum(x$cov1)})
  targetsToFilterOut=intersect(names(s),rownames(offtarget))
  mergedMatrix[,i]=unlist(sumCoveragePerTarget[targetsToFilterOut])
...
# ANGSD
#
# sampleLabelsSwift=unlist(read.table(paste0(WD,"files2/samples.txt"), stringsAsFactors = F))
# angsdAllSampleFileS=paste0(WD,"files2/angsd.anna.originals.depthSample")
# angsdAllGlobalFileS=paste0(WD,"files2/angsd.anna.originals.depthGlobal")
# angsdTargetedSampleFileS=paste0(WD,"files2/angsd.anna.targeted.depthSample")
# angsdTargetedGlobalFileS=paste0(WD,"files2/angsd.anna.targeted.depthGlobal")
# angsdUntargetedSampleFileS=paste0(WD,"files2/angsd.anna.untargeted.depthSample")
# angsdUntargetedGlobalFileS=paste0(WD,"files2/angsd.anna.untargeted.depthGlobal")
#
